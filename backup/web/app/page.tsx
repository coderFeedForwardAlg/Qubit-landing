...
export default function QubitLanding() {
  ...
  const handleWaitlistSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('http://localhost:8081/add_wait_list', {
        ...
        body: JSON.stringify({
          username: waitlistForm.name, // backend expects 'username'
  ...
  const handleHealthCheck = async () => {
    ...
    try {
      ...
      setHealthResult("Error contacting backend")
    }
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 text-white overflow-hidden">
      ...
      {/* Health Check Button for Testing */}
      <div className="p-4 pt-20"> {/* pt-20 ensures it's below the fixed nav */}
        <Button onClick={handleHealthCheck} className="bg-blue-600 hover:bg-blue-700 rounded-xl mr-4">Test Health Endpoint</Button>
        {healthResult && (
          <span className="ml-2 text-lg font-mono">{healthResult}</span>
        )}
      </div>
